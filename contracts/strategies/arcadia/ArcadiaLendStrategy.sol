...
import "../../base/interface/arcadia/ITranche.sol";

/**
 * @title ArcadiaLendStrategy
 * @dev A lending strategy that invests underlying assets into a lending pool, providing yield and rewards.
 */
contract ArcadiaLendStrategy is BaseUpgradeableStrategy {
  ...
  /**
   * @notice Initializes the strategy and verifies compatibility with the Arcadia lending pool.
   * @param _storage Address of the storage contract.
   * @param _underlying Address of the underlying asset.
   * @param _vault Address of the vault.
   * @param _fToken Address of the fToken (Arcadia lending token).
  ...
  function _supply(uint256 amount) internal {
    address _underlying = underlying();
    address _fToken = fToken();
    address lendingPool = ITranche(_fToken).LENDING_POOL();
    IERC20(_underlying).safeApprove(lendingPool, 0);
    IERC20(_underlying).safeApprove(lendingPool, amount);
