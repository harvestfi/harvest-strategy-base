  ...
  */
  ...
  function _redeemWithFlashloan(uint256 amount, uint256 _borrowTargetFactorNumerator) internal {
    ...
    uint256 newBorrowTarget;
    {
        ...
        newBorrowTarget = newBalance.mul(_borrowTargetFactorNumerator).div(uint(1000).sub(_borrowTargetFactorNumerator));
    }
    uint256 borrowDiff;
    ...
    if (borrowDiff > balancerBalance) {
      _redeemNoFlash(amount, supplied, borrowed, _mToken, _borrowTargetFactorNumerator);
    } else {
      address[] memory tokens = new address[](1);
      uint256[] memory amounts = new uint256[](1);
      bytes memory userData = abi.encode(0);
