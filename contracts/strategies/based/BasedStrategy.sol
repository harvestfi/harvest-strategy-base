...
import "../../base/interface/based/IBasedRewards.sol";
...
contract BasedStrategy is BaseUpgradeableStrategy {
  ...
  function initializeBaseStrategy(
    ...
    (address _lpt,,,,) = IBasedRewards(rewardPool()).poolInfo(_poolId);
  ...
  function _rewardPoolBalance() internal view returns (uint256 balance) {
    (balance,) = IBasedRewards(rewardPool()).userInfo(poolId(), address(this));
  }

  function _emergencyExitRewardPool() internal {
    uint256 stakedBalance = _rewardPoolBalance();
    if (stakedBalance != 0) {
      IBasedRewards(rewardPool()).emergencyWithdraw(poolId());
    }
  }

  function _withdrawUnderlyingFromPool(uint256 amount) internal {
    if (amount > 0) {
      IBasedRewards(rewardPool()).withdraw(poolId(), amount);
    }
  }
  ...
  */
  ...
  function _claimRewards() internal {
    IBasedRewards(rewardPool()).withdraw(poolId(), 0);
  }
