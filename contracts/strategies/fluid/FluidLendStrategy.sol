...
import "../../base/interface/fluid/IFluidMerkl.sol";
...
contract FluidLendStrategy is BaseUpgradeableStrategy {
    ...
    address public constant harvestMSIG =
        address(0x97b3e5712CDE7Db13e939a188C8CA90Db5B05131);

    bytes32 internal constant _FTOKEN_SLOT =
        0x462e4d44c9bae3e0ee3d71929710bef82ca7c929ce31980e75572ea415835b0e;
    ...
    constructor() BaseUpgradeableStrategy() {
        assert(
            _FTOKEN_SLOT ==
                bytes32(
                    uint256(keccak256("eip1967.strategyStorage.fToken")) - 1
                )
        );
        assert(
        ...
        assert(
            _PENDING_FEE_SLOT ==
                bytes32(
                    uint256(keccak256("eip1967.strategyStorage.pendingFee")) - 1
                )
        );
    }
    ...
    function initializeBaseStrategy(
        ...
        require(
            IERC4626(_fToken).asset() == _underlying,
            "Underlying mismatch"
        );
        _setFToken(_fToken);
    }
    ...
    function currentBalance() public view returns (uint256) {
        ...
        uint256 underlyingBalance = IERC4626(_fToken).previewRedeem(
    ...
    function totalFeeNumerator() public view returns (uint256) {
        return
            strategistFeeNumerator().add(platformFeeNumerator()).add(
    ...
    function _accrueFee() internal {
        ...
        if (currentBalance() > storedBalance()) {
            ...
            fee = balanceIncrease.mul(totalFeeNumerator()).div(
                feeDenominator()
            );
        }
    ...
    function withdrawToVault(uint256 amountUnderlying) public restricted {
        ...
        IERC20(_underlying).safeTransfer(
            vault(),
            Math.min(amountUnderlying, balance)
        );
        if (balance > 1e2) {
    ...
    function claim(
        ...
        IFluidMerkl(merklContract_).claim(
            address(this),
            cumulativeAmount_,
            positionType_,
            positionId_,
