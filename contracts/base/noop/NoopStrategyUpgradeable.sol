...
contract NoopStrategyUpgradeable is BaseUpgradeableStrategy {
    ...
    function initializeBaseStrategy(address _storage, address _underlying, address _vault) public initializer {
        require(_vault != address(0), "_vault cannot be empty");
        require(_underlying == IVault(_vault).underlying(), "underlying mismatch");

        BaseUpgradeableStrategy.initialize(_storage, _underlying, _vault, address(0), address(0), address(0));
    }
    ...
    */
    function withdrawToVault(uint256 amount) external restricted {
        address _underlying = underlying();
        require(IERC20(_underlying).balanceOf(address(this)) >= amount, "insufficient balance for the withdrawal");
        if (amount > 0) {
