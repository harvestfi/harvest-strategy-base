...
contract RegularVaultFactory is OwnableWhitelist, IVaultFactory {
    ...
    function deploy(address _storage, address underlying) external override onlyWhitelisted returns (address) {
        lastDeployedAddress = address(new VaultProxy(vaultImplementation));
        IVault(lastDeployedAddress).initializeVault(_storage, underlying, 10000, 10000);
    ...
    function info(address vault) external view override returns (address Underlying, address NewVault) {
