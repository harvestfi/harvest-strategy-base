...
interface INFTPool is IERC721 {
  ...
  function getPoolInfo() external view returns (
    address lpToken, address protocolToken, address xToken, uint256 lastRewardTime,
    uint256 accRewardsPerShare, uint256 accRewardsPerShareWETH, uint256 lpSupply,
    uint256 lpSupplyWithMultiplier, uint256 allocPoint, uint256 allocPointsWETH
  );
  function getStakingPosition(uint256 tokenId) external view returns (
    ...
    uint256 rewardDebtWETH, uint256 boostPoints, uint256 totalMultiplier
  );
