...
contract VaultV1 is ERC20Upgradeable, IUpgradeSource, ControllableInit, VaultStorage {
    ...
    function initializeVault(
        ...
        VaultStorage.initialize(_underlying, _toInvestNumerator, _toInvestDenominator, underlyingUnit);
    }
    ...
    */
    ...
    function availableToInvestOut() public view returns (uint256) {
        uint256 wantInvestInTotal = underlyingBalanceWithInvestment().mul(vaultFractionToInvestNumerator()).div(
    ...
    */
    ...
    function _deposit(uint256 amount, address sender, address beneficiary) internal returns (uint256) {
        ...
        uint256 toMint =
            totalSupply() == 0 ? amount : amount.mul(totalSupply()).div(underlyingBalanceWithInvestment().sub(amount));
    ...
    function _withdraw(uint256 numberOfShares, address receiver, address owner) internal returns (uint256) {
        ...
        if (underlyingAmountToWithdraw > underlyingBalanceInVault()) {
            ...
            underlyingAmountToWithdraw = MathUpgradeable.min(
                underlyingBalanceWithInvestment().mul(numberOfShares).div(totalSupply), underlyingBalanceInVault()
            );
        }
