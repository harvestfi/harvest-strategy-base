...
/**
 * @title VaultV1Legacy
 * @dev Exact copy of the VaultV1 implementation currently live behind the regular vault
 * proxies on Base (implementation 0x91CffCc1Fe6B9DA316e8848e141BADB8CD41bA00). The live
 * strategies do not implement the `preInteract`/`doHardWorkOnDeposit` hooks that the
 * current mainline VaultV1 calls, so in-kind upgrades must build on this legacy behavior.
 * The only deviation from the deployed source is that `_deposit` and `_withdraw` are
 * `virtual` so `VaultV2InKind` can sync the strategy's cached balance first while in-kind
 * redemptions are enabled.
 */
contract VaultV1Legacy is ERC20Upgradeable, IUpgradeSource, ControllableInit, VaultStorage {
