...
contract GlobalIncentivesHelper is Controllable {
    ...
    function notifyPools(address[] calldata tokens, uint256[] calldata totals, uint256 timestamp)
        external
        onlyNotifier
    {
    ...
    function newTokenWithHelper(address token, address notifyHelper) public onlyChanger {
        ...
        tokenToHelper[token] = address(
            new NotifyHelperStateful(
    ...
    function setPool(
        ...
        if (notificationType == NotificationType.TOKEN) {
            ...
            NotifyHelperStateful(tokenToHelper[token]).setPool(
                poolAddress,
                poolPercentage,
                NotifyHelperStateful.NotificationType(uint256(NotificationType.FARM)),
                vests
            );
        } else {
            NotifyHelperStateful(tokenToHelper[token]).setPool(
                poolAddress, poolPercentage, NotifyHelperStateful.NotificationType(uint256(notificationType)), vests
            );
        }
