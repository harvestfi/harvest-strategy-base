...
const Strategy = artifacts.require("MorphoVaultStrategyMainnet_EF_USDC");

// Developed and tested at blockNumber 33281400

// Vanilla Mocha test. Increased compatibility with tools that integrate Mocha.
describe("Base Mainnet Morpho Vault ExtraFi USDC", function() {
  let accounts;
  ...
  let extraWhale = "0x89F0885DA2553232aeEf201692F8C97E24715c83";
  ...
  async function setupExternalContracts() {
    ...
    extraToken = await IERC20.at(extra);
  ...
  async function setupBalance(){
    ...
    await web3.eth.sendTransaction({ from: etherGiver, to: extraWhale, value: 10e18});
  ...
  before(async function() {
    ...
    await impersonates([governance, underlyingWhale, extraWhale, morphoWhale]);
    ...
    [controller, vault, strategy] = await setupCoreProtocol({
      "existingVaultAddress": "0x36A4096C6586D25D5B8d7bEC750076443B8b3E60",
  ...
  describe("Happy path", function() {
    it("Farmer should earn money", async function() {
      ...
      for (let i = 0; i < hours; i++) {
        ...
        if (i % 3 == 0) {
          await extraToken.transfer(strategy.address, new BigNumber(1e19), {from: extraWhale});
