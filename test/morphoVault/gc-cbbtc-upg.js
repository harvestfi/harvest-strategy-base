...
const Strategy = artifacts.require("MorphoVaultStrategyMainnet_GC_cbBTC");

// Developed and tested at blockNumber 33281400

// Vanilla Mocha test. Increased compatibility with tools that integrate Mocha.
describe("Base Mainnet Morpho Vault Gauntlet Core cbTBC", function() {
  let accounts;
  ...
  before(async function() {
    ...
    [controller, vault, strategy] = await setupCoreProtocol({
      "existingVaultAddress": "0x6243A06f4BB242E43eEDE2DdA211444C82a61C8B",
  ...
  describe("Happy path", function() {
    it("Farmer should earn money", async function() {
      ...
      for (let i = 0; i < hours; i++) {
        ...
        if (i % 3 == 0) {
          await morphoToken.transfer(strategy.address, new BigNumber(1e19), {from: morphoWhale});
        }
