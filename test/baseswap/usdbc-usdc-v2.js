...
const Strategy = artifacts.require("BaseSwapStrategyV2Mainnet_USDbC_USDC");
const IXBSX = artifacts.require("IXToken");
const IVault = artifacts.require("IVault");

const D18 = new BigNumber(Math.pow(10, 18));

// Developed and tested at blockNumber 4288400

// Vanilla Mocha test. Increased compatibility with tools that integrate Mocha.
describe("Base Mainnet BaseSwap USDbC-USDC", function () {
  let accounts;
  ...
  // external setup
  let underlyingWhale = "0xD3003046F2E7d4E77B9081F30B53E6fe6dc39d47";
  ...
  async function setupExternalContracts() {
    underlying = await IERC20.at("0xC52328d5Af54A12DA68459Ffc6D0845e91a8395F");
  ...
  before(async function () {
    ...
    [controller, vault, strategy, potPool] = await setupCoreProtocol({
      ...
      "liquidation": [{ "baseswap": [bsx, weth, usdbc, usdc] }],
      "ULOwner": ulOwner
    });
  ...
  describe("Happy path", function () {
    it("Farmer should earn money", async function () {
      ...
      for (let i = 0; i < hours; i++) {
        ...
        hodlPrice = new BigNumber(0.7149).times(D18);
        underlyingPrice = new BigNumber(2002078230099.89).times(D18);
