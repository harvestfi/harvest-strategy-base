...
const IPrimePool = artifacts.require("IPrimePool");
...
describe("Base Mainnet DegenPrime USDC", function() {
  ...
  before(async function() {
    ...
    await impersonates([governance, underlyingWhale, roleAdmin]);

    let etherGiver = accounts[9];
    await web3.eth.sendTransaction({ from: etherGiver, to: governance, value: 10e18});
    await web3.eth.sendTransaction({ from: etherGiver, to: roleAdmin, value: 10e18});
    ...
    roleContract = await IPrimePool.at(roleAddr);
    await roleContract.grantRole(roleId, strategy.address, {from: roleAdmin})
